<?php
/**
 * POET_Sniffs_Portability_DatabaseFunctionSniff.
 *
 ...
 * @link      http://git.remote-learner.net/private.cgi?p=codelibrary_scripts.git
 */
class POET_Sniffs_Portability_DatabaseFunctionSniff implements PHP_CodeSniffer_Sniff {
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr) {
        ...
        foreach ($prefixes as $prefix => $type) {
            ...
            if (substr($content, 0, strlen($prefix)) === $prefix) {
                $error = "Portability issue: The function %s will only work with %s and thus can't be used.";
                $data = array($content, $type);
                $phpcsFile->addError($error, $stackPtr, 'Database function', $data);
            }
        }
    }
}
